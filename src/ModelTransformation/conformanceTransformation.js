define([], function () {
  return function (model) {
    const states = []
    for (const state of model.states) { states.push(state) }

    const transitions = []
    let initialState = model.initialState
    for (const transition of model.transitions) { transitions.push(transition) }
