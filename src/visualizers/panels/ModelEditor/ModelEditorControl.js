...
define(['js/logger',
  ...
  const BACKGROUND_TEXT_COLOR = '#DEDEDE'
  const BACKGROUND_TEXT_SIZE = 30
  const DEFAULT_DECORATOR = 'ModelDecorator'
  const WIDGET_NAME = 'DiagramDesigner'
  const SRC_POINTER_NAME = CONSTANTS.POINTER_SOURCE
  const DST_POINTER_NAME = CONSTANTS.POINTER_TARGET

  const ModelEditorControl = function (options, config) {
  ...
  ModelEditorControl.prototype.selectedObjectChanged = function (nodeId) {
    ...
    if (nodeId || nodeId === CONSTANTS.PROJECT_ROOT_ID) {
      ...
      if (this._selectedAspect !== CONSTANTS.ASPECT_ALL) {
        ...
        const aspectNames = node ? node.getValidAspectNames() : []
  ...
  ModelEditorControl.prototype._getObjectDescriptor = function (nodeId) {
    const self = this
    const nodeObj = this._client.getNode(nodeId)
    let objDescriptor
    let customPoints
  ...
  ModelEditorControl.prototype._eventCallback = function (events) {
    const i = events ? events.length : 0
    let refresh
  ...
  ModelEditorControl.prototype.processNextInQueue = function () {
    let nextBatchInQueue
    let len = this.eventQueue.length
    const decoratorsToDownload = [DEFAULT_DECORATOR]
    let itemDecorator
    let refresh = false
    const self = this
  ...
  ModelEditorControl.prototype._dispatchEvents = function (events) {
    ...
    const self = this
    let orderedItemEvents
    let orderedConnectionEvents
    let unloadEvents
    ...
    if (this._firstLoad === true) {
      ...
      const activeSelection = WebGMEGlobal.State.getActiveSelection()
  ...
  ModelEditorControl.prototype._addGMEConnection = function (gmeID, objDesc) {
    const uiComponent = this.designerCanvas.createConnection(objDesc)
  ...
  ModelEditorControl.prototype._onUnload = function (gmeID) {
    let componentID
    let len
    let getDecoratorTerritoryQueries
    const self = this
    let territoryChanged = false
  ...
  ModelEditorControl.prototype._removeConnectionSegmentPoints = function () {
    const idList = this.designerCanvas.selectionManager.getSelectedElements()
  ...
  ModelEditorControl.prototype._stateActiveSelectionChanged = function (model, activeSelection, opts) {
    let selectedIDs = []
    let len = activeSelection ? activeSelection.length : 0
  ...
  ModelEditorControl.prototype._updateTopNode = function () {
    const projectId = this._client.getActiveProjectId()
    let projectKind
  ...
  ModelEditorControl.prototype._displayToolbarItems = function () {
    if (this._toolbarInitialized !== true) {
      ...
      for (let i = 0; i < this._toolbarItems.length; i++) {
  ...
  ModelEditorControl.prototype._updateAspects = function () {
    const nodeId = this.currentNodeInfo.id
    const nodeObj = this._client.getNode(nodeId)
    let aspects
    let tabID
    let i
    let selectedTabID
    const activePanel = WebGMEGlobal.PanelManager.getActivePanel()
    ...
    if (this._selectedAspect !== CONSTANTS.ASPECT_ALL) {
      const newAspectRules = this._client.getAspectTerritoryPattern(nodeId, this._selectedAspect)
      let aspectRulesChanged = false
