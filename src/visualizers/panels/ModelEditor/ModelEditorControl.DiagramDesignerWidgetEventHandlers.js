...
define(['js/logger',
  ...
  const REGISTRY_STRING = 'registry'
  const SRC_POINTER_NAME = CONSTANTS.POINTER_SOURCE
  const DST_POINTER_NAME = CONSTANTS.POINTER_TARGET

  const ModelEditorControlDiagramDesignerWidgetEventHandlers = function () {}
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._saveNewPositions = function (repositionDesc) {
    const self = this
    const gmeIds = Object.keys(repositionDesc)
    const modelId = this.currentNodeInfo.id
    let gmeId
    let i
    let newPos
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._addConnSegmentPoints = function (posInfo) {
    ...
    const self = this
    const connIds = WebGMEGlobal.State.getActiveSelection().filter(function (gmeId) {
      const nodeObj = self._client.getNode(gmeId)
      return nodeObj && nodeObj.isConnection()
    })

    connIds.forEach(function (gmeId) {
      const nodeObj = self._client.getNode(gmeId)
      const linePoints = nodeObj && nodeObj.getRegistry(REGISTRY_KEYS.LINE_CUSTOM_POINTS)
      ...
      if (linePoints instanceof Array && linePoints.length > 0) {
        ...
        let srcNode = nodeObj.getNode(nodeObj.getPointerId('src'))
        let dstNode = nodeObj.getNode(nodeObj.getPointerId('dst'))
        ...
        if (srcNode && posInfo[srcNode.getId()] && dstNode && posInfo[dstNode.getId()]) {
          const srcPos = srcNode.getRegistry('position')
          const dstPos = dstNode.getRegistry('position')
          const delta = { x: 0, y: 0 }
          let pos
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemsCopy = function (copyDesc) {
    const copyOpts = { parentId: this.currentNodeInfo.id }
    let id
    let desc
    let gmeID
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onCreateNewConnection = function (params) {
    ...
    let sourceId
    let targetId
    const parentId = this.currentNodeInfo.id
    let createConnection
    const _client = this._client
    const CONTEXT_POS_OFFSET = 10
    const menuItems = {}
    let i
    let connTypeObj
    const aspect = this._selectedAspect
    let newConnID
    let validConnectionTypes
    let dstPosition

    function getMidPosition () {
      const srcPos = self.designerCanvas.items[params.dst].getBoundingBox()
      const dstPos = self.designerCanvas.items[params.src].getBoundingBox()
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDesignerItemDoubleClick = function (id /*, event */) {
    const gmeID = this._ComponentID2GMEID[id]
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onModifyConnectionEnd = function (params) {
    const gmeID = this._ComponentID2GMEID[params.id]
    const oldDesc = params.old
    const newDesc = params.new
    let newEndPointGMEID
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onUnregisterSubcomponent = function (objID,
    sCompID) {
    const gmeID = this._Subcomponent2GMEID[objID][sCompID]
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getPossibleDropActions = function (dragInfo) {
    ...
    const dragParams = DragHelper.getDragParams(dragInfo)
    const possibleDropActions = []
    const parentID = this.currentNodeInfo.id
    let i
    let j
    let FCOamongItems = false
    let validPointerTypes = []
    let baseTypeID
    let baseTypeNode
    let dragAction
    const aspect = this._selectedAspect
    const pointerSorter = function (a, b) {
      const baseAName = a.name.toLowerCase()
      const baseBName = b.name.toLowerCase()
      const ptrAName = a.pointer.toLowerCase()
      const ptrBName = b.pointer.toLowerCase()
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onBackgroundDrop = function (event, dragInfo,
    position) {
    const possibleDropActions = this._getPossibleDropActions(dragInfo)
    const len = possibleDropActions.length
    let i
    let selectedAction
    const self = this
    let menuItems
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._handleDropAction = function (dropAction, dragInfo,
    position) {
    const self = this
    const dragEffect = dropAction.dragEffect
    const items = DragHelper.getDragItems(dragInfo)
    const dragParams = DragHelper.getDragParams(dragInfo)
    const parentID = this.currentNodeInfo.id
    let positionInfo
    ...
    function getParams () {
      const result = { parentId: parentID }
      i = items.length
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getNewPositionFromDrag = function (items,
    dragPositions,
    dropPosition) {
    const result = {} // {<gmeId>: {x: <int>, y <int>}

    items.forEach(function (id) {
      const relPos = dragPositions && dragPositions[id]
      const newPos = { x: dropPosition.x, y: dropPosition.y }
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionChanged = function (selectedIds) {
    ...
    const gmeIDs = []
    const extraIds = []
    let enableEditConnections = selectedIds.length > 0

    selectedIds.forEach(function (id) {
      const gmeId = self._ComponentID2GMEID[id]
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onConnectionSegmentPointsChange = function (params) {
    ...
    const gmeID = this._ComponentID2GMEID[connID]
    let nodeObj
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterNewConnectionDroppableEnds = function (params) {
    const availableConnectionEnds = params.availableConnectionEnds
    const result = []
    let i
    let sourceId
    let targetId
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onFilterReconnectionDroppableEnds = function (params) {
    ...
    const srcDragged = params.draggedEnd === DiagramDesignerWidgetConstants.CONNECTION_END_SRC
    let srcItemID = params.srcItemID
    let srcSubCompID = params.srcSubCompID
    let dstItemID = params.dstItemID
    let dstSubCompID = params.dstSubCompID
    const availableConnectionEnds = params.availableConnectionEnds
    const availableConnectionSources = params.availableConnectionSources
    let i
    const result = []
    let newEndPointGMEID
    let oldEndPointGMEID
    const connectionGMEID = this._ComponentID2GMEID[connID]
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onDragStartDesignerItemCopyable = function (itemID) {
    const nodeObj = this._client.getNode(this._ComponentID2GMEID[itemID])
    let result = true
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSetConnectionProperty = function (params) {
    ...
    const visualParams = params.params
    const gmeIDs = []
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onCopy = function () {
    const res = []
    const selectedIDs = this.designerCanvas.selectionManager.getSelectedElements()
    let i = selectedIDs.length
    let gmeID
    let obj
    let nodeObj
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onPaste = function (data) {
    ...
    const projectName = this._client.getActiveProjectId()
    const childrenIDs = []
    const aspect = this._selectedAspect
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._getDragParams = function (selectedElements, event) {
    const oParams = this._oGetDragParams.call(this.designerCanvas, selectedElements, event)
    const params = {
      positions: {},
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionContextMenu = function (selectedIds,
    mousePos) {
    const menuItems = {}
    const MENU_CONSTRAINTS_NODE = 'connode'
    const MENU_CONSTRAINTS_MODEL = 'conmodel'
    const MENU_META_RULES_NODE = 'metaRulesNode'
    const MENU_META_RULES_MODEL = 'metaRulesModel'
    const MENU_EDIT_REPLACEABLE = 'editReplaceable'
    const MENU_EXPORT_MODELS = 'exportModels'
    const MENU_IMPORT_MODELS = 'importModels'
    let node
    let i
    let paths
    let libraryContentSelected = false
    const self = this
    ...
    this.designerCanvas.createMenu(menuItems, function (key) {
      if (key === MENU_CONSTRAINTS_NODE) {
        ...
        const importDialog = new ImportModelDialog(self._client, self.logger.fork('ImportModel'))
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onSelectionAlignMenu = function (selectedIds,
    mousePos) {
    const menuPos = this.designerCanvas.posToPageXY(mousePos.mX, mousePos.mY)
    const self = this
    const itemsIds = selectedIds.filter(function (itemId) {
  ...
  ModelEditorControlDiagramDesignerWidgetEventHandlers.prototype._onAlignSelection = function (selectedIds, type) {
    ...
    function getItemData (itemId) {
      const item = self.designerCanvas.items[itemId]
