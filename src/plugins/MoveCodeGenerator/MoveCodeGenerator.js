...
define([
  ...
  MoveCodeGenerator.getGeneratedFiles = function (self, nodes, activeNode, callback) {
    ...
    const violations = MoveCodeGenerator.prototype.getViolations.call(self, contractPaths, nodes)
    const fileNames = []
    let fileName
    const promises = []
    let type
  ...
  MoveCodeGenerator.prototype.parseResult = function (contractNode, fileContent, violations) {
    const self = this
    const tree = self.parser.parse(fileContent)
  ...
  MoveCodeGenerator.prototype.getViolations = function (contracts, nodes) {
    const contractNames = {}
    let name; let type; let node
    let child; let childPath; let childName
    const self = this
    let noInitialState; let noCreateTransition
    let nameAndViolations = {
      violations: [],
      totalStateNames: {},
      transitionNames: {}
    }
    ...
    for (type of contracts) {
      ...
      if (Object.prototype.hasOwnProperty.call(contractNames, name)) {
  ...
  MoveCodeGenerator.prototype.hasChildViolations = function (child, childName, nameAndViolations) {
    ...
    if ((self.isMetaTypeOf(child, self.META.State)) || (self.isMetaTypeOf(child, self.META.InitialState))) {
      if (Object.prototype.hasOwnProperty.call(nameAndViolations.totalStateNames, childName)) {
        nameAndViolations.violations.push({
          node: child,
    ...
    if (self.isMetaTypeOf(child, self.META.Transition) || self.isMetaTypeOf(child, self.META.CreateTransition)) {
      if (Object.prototype.hasOwnProperty.call(nameAndViolations.transitionNames, childName)) {
        nameAndViolations.violations.push({
          node: child,
